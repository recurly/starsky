...
function Queue (name, rabbit) {
  this.name = name;
  this.rabbit = rabbit;
  this.logger = rabbit.logger;
  this.subscriptions = [];
  this.callback = null;
  this.queue = null;
}
...
Queue.prototype.consume = function (callback) {
  if (!this.rabbit.ready) throw new Error('not ready');

  this.callback = callback;

  var exchange = this.rabbit.get('mq exchange');
  var connection = this.rabbit.connection;
